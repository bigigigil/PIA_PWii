    ...
    if (token) {
        try {
            ...
            if (response.ok) {
                ...
                if (formPerfil) {
                    document.getElementById('tituloNombre').textContent = `Hola, ${usuario.nombre}`;
                    document.getElementById('avatarPerfil').src = `https://ui-avatars.com/api/?name=${usuario.nombre.replace(' ', '+')}&background=1a4d8c&color=fff&rounded=true`;
